...
<<<<<<< HEAD
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS needed_items (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                        item_name TEXT NOT NULL,
                        description TEXT,
                        urgency TEXT DEFAULT 'Medium'
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS dropoff_locations (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                        location_name TEXT NOT NULL,
                        address TEXT NOT NULL,
                        hours TEXT NOT NULL
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS volunteer_roles (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                        role_title TEXT NOT NULL,
                        description TEXT NOT NULL
                    )''')
...
<<<<<<< HEAD
    if request.method == 'POST':
        ...
        c = conn.cursor()
        try:
            c.execute("INSERT INTO donors (name, email, password, food_bank_name, address) VALUES (?, ?, ?, ?, ?)",
                      (name, email, password, food_bank_name, address))
=======
    if request.method == "POST":
        name = request.form["name"]
        email = request.form["email"]
...
<<<<<<< HEAD
    if 'user' in session and session.get('user_type') == 'donor':
        ...
        donor_dashboard_data = {
            ...
            "food_received": "540 kg",
            "active_requests": 3,
            "donors_connected": 42,
            "volunteers_managed": 8,
            "donor_activity": [
                ("Received Canned Goods from John D.", "1 hour ago"),
                ("Received 10kg Pasta from Jane S.", "5 hours ago")
            ]
        }
