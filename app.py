...
def register():
    ...
    # Check if email exists in users or donors
    c.execute("SELECT 1 FROM users WHERE email=?", (email,))
    exists_in_users = c.fetchone()

    c.execute("SELECT 1 FROM donors WHERE email=?", (email,))
    exists_in_donors = c.fetchone()

    if exists_in_users or exists_in_donors:
        flash("⚠️ This email is already in use. Please use another email.", "error")
        conn.close()
        return redirect(url_for("login_page"))
